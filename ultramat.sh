#!/bin/bash
#Creado Por CyberPunk
#En Colaboración continua para Github
...
echo -e " ${YELLOW}
     ...
     ',:::                       ,'	
       ' .:::                 , '	
           ' · . . . . . . · '	"	
echo -e ""
echo -e "\t${RED}Utilice: ${BIGreen}$0 <Dirección IP> <Tipos de Escaneos>"
echo -e 
...
echo -e "\t${BIGreen}Vulne	${BIBlue}:Busca Vulnerabilidades CVE en todos los puertos encontrados (5 a 15 minutos)"
echo -e "\t${BIGreen}Recon	${BIBlue}:Recomienda comandos de reconocimiento WEB o los ejecuta automáticamente (Instale GOBUSTER v3.0)"
...
header(){
echo -e "${BIBlue}Creado Por: CyberPunk"
echo -e ""
if [ "$2" == "Todo" ]; then
...
echo -e "${NC}"
$nmapType -sV --script vuln -p`echo "${ports}"` -oN nmap/Vulne_$1.nmap $1
echo -e ""
echo -e ""
echo -e ""
}
...
if [[ "$2" =~ ^(Rapido|Basico|UDP|Completo|Vulne|Recon|Todo)$ ]]; then
	if [[ ! -d $1 ]]; then
	        mkdir $1
	fi
	...
	case "$2" in
		...
		UDP) 	UDPScan $1;;
		Completo) 	fullScan $1;;
		Vulne) 	if [ ! -f nmap/Rapido_$1.nmap ]; then rapidoScan $1; fi
			vulnsScan $1;;
